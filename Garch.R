...
plot(rtd.SP)
Box.test(rtd.SP, lag=12, type='Ljung-Box') # no autocorrelation
Box.test(rtd.SP^2, lag=12, type='Ljung-Box') # no autocorrelation

FinTS::ArchTest(x=rtd.SP, lags=12)  # significant autocorrelation
...
### Use ruGarch package
require(rugarch)
spec=ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1, 1)), # try eGarch Model
...
legend("top", c('gjrGarch','sGarch'), col = c(1,3), lty = c(1, 1), bty = "n")
...
legend("top", c('BS formula','call option Ask'), col = c(1,3), lty = c(1, 1), bty = "n")
...
plot(OP.strick,CP[2,], col=3,xlab='Strick Price', ylab='Stock Price', 
     type = "l", ylim = c(0,2000), lwd=2, pch=24)
abline(v=P, col='red', lty=3)
legend("top", c('BS formula','put option Ask'), col = c(1,3), lty = c(1, 1), bty = "n")
